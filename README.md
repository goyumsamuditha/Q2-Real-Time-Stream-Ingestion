...
*(Add screenshot here: Terminal showing the Docker Compose up command and the Kafka topic creation commands)*
...
*(Add screenshot here: Producer terminal showing the active stream and the "Simulating out-of-order event..." logs)*
...
*(Add screenshot here: Flink Web Dashboard at localhost:8082 showing the pipeline in a RUNNING state)*
...
*(Add screenshot here: PyFlink code snippet showing the SQL query and the 15-minute TUMBLE function)*
...
*(Add screenshot here: Kafka UI showing the populated traffic-window-results topic with the final aggregated JSON messages)*
...
Navigate to the Flink job directory, install dependencies, and launch the stream processor:
